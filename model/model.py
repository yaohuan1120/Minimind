...
def precompute_freqs(
    ...
    rope_base: float = 1e6,
    rope_scaling: Optional[dict] = None, # 可选的字典参数，用于调整 RoPE 频率
):
    # 写出最初 RoPE 频率预计算函数
    ...
    if rope_scaling is not None:
        ...
        if end / original_max > 1.0:   # 嵌套 if 判断，end 是推理最大长度，original_max 是训练长度, 只有当推理长度超过训练长度时才进行缩放
                # 计算corr_dim
                corr_dim = next(
    ...
    # 返回一个 cos 和 sin 的频率矩阵，形状为 (end, dim)
...
class Attention(nn.Module):
    ...
    # 前向传播函数，接受输入张量 x、位置编码 position_embeddings、可选的 past_key_value 和 attention_mask
    def forward(
        self,
        x: torch.Tensor,
        ...
        xq, xk = apply_rotary_pos_emb(xq, xk, cos[:seq_len], sin[:seq_len]) # 将查询和键张量与对应的 RoPE 频率进行旋转位置编码

        # 如果提供了 past_key_value，则将其与当前的键和值张量连接起来，以支持自回归生成
        ...
        # 转置查询、键、值张量以适应注意力计算，并根据 n_rep 重复键和值张量以匹配查询的头数
        xq = xq.transpose(1, 2)
        ...
        if (
            ...
            output = scores @ xv   # 注意力权重与值张量相乘得到输出

        # 转置输出张量并通过线性层进行最终变换，最后应用残差连接和 dropout
        output = output.transpose(1, 2).reshape(bsz, seq_len, -1)
        output = self.resid_dropout(self.o_proj(output))
        return output, past_kv


# 定义前馈网络类，继承 nn.Module
...
class MokioMindModel(nn.Module):
    def __init__(self, config: MokioMindConfig):
        ...
        # 预计算 RoPE 频率并注册为 buffer，以便在前向传播中使用
